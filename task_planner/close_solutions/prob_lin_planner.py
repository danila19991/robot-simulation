...
def generate_prob_lin_solutions(scenario: ExampleScenario, k: int, a):
    a = a.reshape((4, 4))
    ...
    for cart_len in scenario.cart_lengths:
        if len(res) < k:
            q, v = generate_min_prob_lin_one(scenario.manip_lengths, cart_len, res, vals, ns, a)
        else:
            q, v = generate_min_prob_lin_one(scenario.manip_lengths, cart_len, res[-k:], vals[-k:], ns, a)
        res.append(q)
        vals.append(v)
    # print("----", res)
    return res
